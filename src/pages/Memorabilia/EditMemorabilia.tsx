...
import type { MemorabiliaCreate, MemorabiliaUpdate } from '../../services/api';
...
export default function EditMemorabilia() {
  ...
  const { data: memorabiliaItem, loading: loadingItem } = useApi(
    () => id ? apiService.getMemorabilia({ limit: 1, id }) : Promise.resolve(null),
    { 
      immediate: !!id,
      cacheKey: `edit-memorabilia-${id}`,
      cacheTTL: 2 * 60 * 1000,
      staleWhileRevalidate: true
    }
  );

  const { mutate: updateMemorabilia, loading } = useMutation(
    (data: { id: string; data: MemorabiliaUpdate }) => apiService.updateMemorabilia(data.id, data.data)
  );
  ...
  useEffect(() => {
    if (memorabiliaItem?.rows?.[0]) {
      const item = memorabiliaItem.rows[0];
      setFormData({
        title: item.title || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title.trim() || !id) {
      error('Error', 'Title is required and memorabilia ID must be provided.');
      return;
    }
    try {
  ...
  return (
    <div className="max-w-2xl mx-auto py-10">
      <h1 className="text-2xl font-bold mb-6">Edit Memorabilia</h1>
