...
export default function MemorabiliaPage() {
  ...
  const { data: allMemorabiliaData, loading, execute: refetchMemorabilia } = useApi(
    ...
    { 
      immediate: true,
      cacheKey: `memorabilia-${productSlug || 'all'}-${searchTerm}-${JSON.stringify(selectedFilters)}-${sortBy}`,
  ...
  // Filter memorabilia based on product relationship
  const allItems = React.useMemo(() => {
    const items = allMemorabiliaData?.rows || [];
    
    // If we have a product slug, only show memorabilia related to that product
    ...
    return items;
  }, [allMemorabiliaData, productSlug, productData]);
      ...
      <div className="memorabilia-page-header bg-gray-50 py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-4 md:px-6 text-center">
          <h1 className="text-5xl font-bebas text-gray-900 mb-4" style={{ fontSize: '48px' }}>
            {productData ? `${productData.title} - Memorabilia` : 'Movie Memorabilia'}
          </h1>
          <p className="text-gray-600 font-inter">
            {productData ? `Authentic memorabilia from ${productData.title}` : 'Authentic pieces from your favorite films'}
          </p>
          {productData && (
            <div className="mt-4">
              <button
