...
export default function Homepage() {
  ...
  const { data: productsDataSlider } = useApi(
    () => apiService.getProducts({ limit: 8, is_on_homepage_slider: true }),
    { 
      immediate: true,
      cacheKey: 'homepage-products',
      cacheTTL: 1 * 60 * 1000, // 10 minutes for homepage
      staleWhileRevalidate: true
    }
  );
   const { data: productsDataTrending } = useApi(
    () => apiService.getProducts({ limit: 12, is_trending_model: true }),
    { 
      immediate: true,
  ...
  // Use API data
  const productsForSlider = productsDataSlider?.rows || [];
  const productsForTrending = productsDataTrending?.rows || [];

  // Memoized filtered products for performance
  const homepageSliderProducts = useMemo(
    () => Array.isArray(productsForSlider) ? productsForSlider : [],
    [productsForSlider]
  );
  const trendingModelsProducts = useMemo(
    () => Array.isArray(productsForTrending) ? productsForTrending : [],
    [productsForTrending]
  );
  ...
  const slides = [
    {
      ...
      image: '/reel_wheel_hero_first_slide.webp',
      video: null
    },
    ...homepageSliderProducts.map(product => ({
      type: 'product',
      id: product.id,
      title: product.title,
      subtitle: product.subtitle || '',
      image: product.images?.[0] || '/vdp_hero.webp',
...
function TrendingModelsSlider({ products, navigate }) {
  ...
  // Use all trending models, do not filter by is_on_homepage_slider
